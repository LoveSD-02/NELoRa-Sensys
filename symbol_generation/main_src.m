...
fileID = fopen('input/data_colllection_sf7', 'r');
...
while ~isempty(pre_data) && ex < 2
    ...
    if frame_sign
        ...
        frame_decoder_comp(sig, [filepath, 'raw/', num2str(ex), '_demod.csv']);
        ...
        for i = 1 : floor(numel(upayload)/nsamp)
            ...
            % write file
            filename = sprintf('%d_%d_34_%d_%d_%d.mat', i, round(value), SF, BW, instance);
            io_write_iq([filepath,filename], symb);
